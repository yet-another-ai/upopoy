...
const search = useGlobalSearch({ limit: props.limit, type: props.type })
...
const panelClass = computed(() =>
  props.panelWidth === 'input'
    ? 'left-0 right-0 w-full'
    : 'right-0 w-[min(32rem,calc(100vw-2rem))]',
)
...
<template>
  <div ref="searchRoot" class="relative">
    ...
    <div
      v-if="showPanel"
      class="bg-popover text-popover-foreground ring-foreground/10 absolute top-full z-50 mt-2 grid max-h-[min(28rem,calc(100vh-5rem))] overflow-hidden rounded-lg shadow-lg ring-1"
      :class="panelClass"
    >
      <div
      ...
      <div v-else-if="!hasResults" class="text-muted-foreground px-3 py-2 text-sm">No results</div>
